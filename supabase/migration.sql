...
CREATE POLICY "Members can view family" ON families FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM family_members WHERE family_members.family_id = families.id AND family_members.user_id = auth.uid()));
CREATE POLICY "Anyone can view family by invite code" ON families FOR SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated users can create families" ON families FOR INSERT TO authenticated WITH CHECK (auth.uid() = created_by);
CREATE POLICY "Family admin can update family" ON families FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM family_members WHERE family_members.family_id = families.id AND family_members.user_id = auth.uid() AND family_members.role = 'admin'));
CREATE POLICY "Family admin can delete family" ON families FOR DELETE TO authenticated
  USING (auth.uid() = created_by);

-- Family members policies
CREATE POLICY "Members can view members" ON family_members FOR SELECT TO authenticated
  USING (EXISTS (SELECT 1 FROM family_members fm WHERE fm.family_id = family_members.family_id AND fm.user_id = auth.uid()));
CREATE POLICY "Authenticated users can join families" ON family_members FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Family admin can remove members" ON family_members FOR DELETE TO authenticated
  USING (
    user_id = auth.uid() OR
    EXISTS (SELECT 1 FROM family_members fm WHERE fm.family_id = family_members.family_id AND fm.user_id = auth.uid() AND fm.role = 'admin')
  );

-- Helper function: check if two users are in the same family
CREATE OR REPLACE FUNCTION are_family_members(user_a uuid, user_b uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
