...
export default function ImportRecipePage() {
  ...
  async function handleSave() {
    if (!userId) return
    if (!title.trim()) { toast.error('חובה להזין שם מתכון'); return }
    if (ingredients.filter((i) => i.name.trim()).length === 0) { toast.error('חובה להזין מצרך אחד לפחות'); return }
    if (steps.filter((s) => s.trim()).length === 0) { toast.error('חובה להזין שלב אחד לפחות'); return }
